...
optimizer = optim.SGD(model1.parameters(), lr=hyperparameters['lr'], weight_decay=hyperparameters['weight_decay'])
log_interval = 10
...
def retrain():
    global train_data_set,model1,model2
    print("retrain start----")
    ...
    for epoch in range(1, 10):
        train(model=model1, epoch=epoch, train_loader=train_loader)
        test(model=model1, epoch=epoch, test_loader=train_loader)
    return
...
def classification(audio_data, video_data ,model1,model2):
    global device
    label = 0
    ...
    #---------------Pytorch LSTM--------------------
    output1, hidden_state1 = model1(final_data, model1.init_hidden(hyperparameters["batch_size"]))
    output2, hidden_state1 = model2(final_data, model2.init_hidden(hyperparameters["batch_size"]))
    pred = torch.max(output2, dim=1).indices
    label = pred
    #print(label)
    print("model 1 new method is {} and old is{}".format(output1[0][1],label))
    # --------------tensorflow alexnet-------------------
    # if not data:
    #     return 0
    # arr = np.array(data)
    ...
    # print(label)
    if args.feedback:
        store_data(audio_data, video_data, output1)
